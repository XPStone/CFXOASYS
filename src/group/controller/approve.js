...
module.exports = class extends Base {
    ...
    async contentAction() {
        ...
        let userList = await this.model('user').where({group_id : this.groupId}).select()
        list.forEach(d => {
            const opdata = userList.find(e => e.id == d.op_id)
            if(opdata) {
                d.op_name = opdata.name
            }
            const userdata = userList.find(e => e.id == d.user_id)
            if(userdata) {
                d.user_name = userdata.name
            }
            const infdata = dataType.find(e => e.id == d.type)
            if(infdata) {
                d.infoDetail = infdata.info
            }
    ...
    async msgListAction() {
        ...
        list.forEach(d => {
            const data = userList.find(u => u.id == d.to_user_id)
            if(data) {
                d.name = data.name
            }
